...
//#define gameRecWidth  220       // Width in pixels of game record (###
//SHOULDN'T BE HERE???)
...
#define ecoStrLen \
  10  // Actually 6 is enough (XDD or XDD/DD), but just in case...
 ...
 * -----------------------------------*/
// "Hard" (automatically computed) results; when a new game move has been
// performed The game can NOT continue in these cases:
...
// "Soft" (manually set) results; in case of draw accepted, resignation or time
// forfeit The game can still continue in these cases.
...
// Game map entry flags (upper bits of GameMap[].moveNo) indicating type of
// line:
...
// If the gameMap_Special flag is set in GameMap[n].moveNo, then the following
// contents of the the GameMap[n].txLine describes the actual contents of the
// special line:

#define gameMap_SpecialBlank 0  // Blank line
#define gameMap_SpecialChapter \
  1  // Collection game title as chapter (bold, larger)
#define gameMap_SpecialSection \
  2  // Collection game title as section (bold, larger)
...
typedef struct {
  ...
  // Sigma specific lauout info for basic desktop publishing based on game
  // collections (stored using non-standard PGN tags):
...
typedef struct {
  ...
  BYTE Game[];  // Null move terminated list of moves. >= 4 bytes pr move.
                ...
                // begins is a null terminated comment string (prior to the
                // move).
} GAMEDATA2;
...
class CGame {
 public:
  ...
  void PlayMoveRaw(
      MOVE *m);  // Add new move to game record (no flags, disamb etc)
  ...
  void Edit_SetPiece(SQUARE sq,
                     PIECE p);  // Set the piece "p" on the square "sq".
  void Edit_MovePiece(SQUARE from,
                      SQUARE to);  // Moves contents of square "from" to "to".
  ...
  void SetAnnotation(INT moveNo, CHAR *text, INT size,
                     BOOL killNewLines = false);
  ...
  INT CalcGameMap(INT toMove, GAMEMAP GMap[], BOOL isPrinting,
                  BOOL isCollectionGame, BOOL isPublishing);
  void InsertAnnGameMap(INT j, INT *N, GAMEMAP GMap[]);
  INT InsertGameMapHeader(GAMEMAP GMap[], BOOL isPrinting,
                          BOOL isCollectionGame, BOOL isPublishing);
  ...
  MOVE Record[gameRecSize];  // The actual game record containing the moves.
                             // Record[0] contains a dummy null move. The most
                             // recently played move is thus stored in
                             // Record[currMove].

  DRAWDATA
      DrawData[gameRecSize +     // The draw information table. Auxiliary table
                                 // used to check
               maxSearchDepth];  // draw by repetition and draw by the 50 move
                                 // rule. Is indexed along with Record[]. The
                                 // "hashKey" field of "DrawTab[i]" is a 32 bit
                                 // hash key of the position resulting from the
                                 // move "Record[i]". The "repCount" field
                                 // indicates the number of times the i'th
                                 // position has occurred. The "irr" field is
                                 // the index in Record[]/DrawTab[] of the last
                                 // irreversible move (incl. the last move) and
                                 // is used to check draw by the 50 move rule
                                 // (as well as to facilitate repetition check).

  //--- Current board position (incrementally updated) ---

  PIECE Board_[boardSize1],  // Current board configuration (position after
                             // current move).
      Board[boardSize2];
  ...
  MOVE
      Moves[maxLegalMoves];  // Strictly legal moves in current position. Should
                             // always be kept up to date with current position.
  ...
  BOOL kingInCheck;  // Is players king in check? (computed during move
                     // generation).
  ...
  BOOL
      hasOfferedDraw;  // Has program already offered draw? (###store move no??)
 ...
 private:
  ...
  // This makes use of the PACKET struct for the standard Sigma Chess game
  // format.
