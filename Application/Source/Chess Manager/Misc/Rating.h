...
typedef struct
{
   ...
   BOOL autoReduce;       // Automatically decrease ELO if Computer not fast enough? (default OFF!!)
} ENGINE_RATING;
...
typedef struct
{
   ...
   UINT history[maxRatingHistoryCount];  // Bit 15     : player colour (0=white, 1=black)
                                         // Bits 14-13 : result (0=loss, 1=draw, 2=win)
                                         // Bits 11-0  : Sigma ELO (0-4095)
} PLAYER_RATING;
