...
// Each game window is associated with a CLevel object describing the playing
// modes, levels etc which applies to that window. Additionally, the most
// recently edited CLevel object is stored in the preferences file and is used
// as the default when creating new game windows. All times are specified in
// seconds.
...
typedef struct {
  ...
  struct {
    ...
    INT delta;      // Time gain per move (in seconds) if Fischer or Bronstein
                    // clock.
  } TimeMoves;
  ...
  struct {
    ...
    INT moves[3];  // Moves to play within each time control (always all in last
                   // time control).
  } Tournament;
  ...
  struct {
    ...
    INT scoreLimit;  // Search stops when this limit exceeded (maxVal) if no
                     // limit.
  } Solver;
