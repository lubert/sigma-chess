...
{
	// Parameters
   UCI_ENGINE_ID engine1;     // White in first game (all games if alternate = false)
   UCI_ENGINE_ID engine2;     // Black in first game (all games if alternate = false)
...
typedef struct
{
   ...
   struct  // Filter computer annotations in "Analyze Game" and "Analyze Collection":
   {
      INT   timePerMove;
      BOOL  skipWhitePos;
      BOOL  skipBlackPos;
   ...
   //----- GAME WINDOW PREFS -----
   // These prefs are specific to each game window. The most recently used values are
   // used when creating new game windows:
   ...
   struct   // Level settings:
   {
      ...
      BOOL     permanentBrain_OBSOLETE;        // OBSOLETE (stored in Prefs.UCI for each engine instead)
      BOOL     nonDeterm;
      ENGINE_RATING strength_OBSOLETE;         // OBSOLETE (stored in Prefs.UCI for each engine instead)
      INT      playingStyle;
      INT      unused[8];
   } Level;
