...
void UCI_SetPonderOption (UCI_ENGINE_ID engineId, BOOL ponder);                          // Only sent if changed or flush
void UCI_SetStrengthOption (UCI_ENGINE_ID engineId, BOOL limitStrength, INT engineELO);  // Only sent if changed or flush
...
INT  UCI_GetMultiPVOptionId (UCI_ENGINE_ID engineId); // Returns -1 if not supported
...
void UCI_OptionValueToStr (UCI_OPTION *Option, CHAR *str); // For display purposes only
